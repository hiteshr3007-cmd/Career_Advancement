...
| `GET /api/v1/auth/me` + `POST /api/v1/auth/register` (concurrent) | stress | 500 OperationalError on Neon `-pooler` | Double pooling (PgBouncer + QueuePool) — **N/A for local Docker Postgres**; only relevant if pointing at a pooled URL later | NOT APPLICABLE (local Docker) |
