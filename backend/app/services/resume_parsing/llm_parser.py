# --- OpenAI LLM resume parsing fallback (disabled for now, no OpenAI account/credits assumed) ---
# The rules-based parser (rules_parser.py) handles the common case; this was
# only a fallback for low-confidence extractions. Re-enable by uncommenting
# everything below and restoring parse_resume_with_llm's body.
#
# import json
#
...
def parse_resume_with_llm(resume_text: str) -> dict:
    """LLM fallback is disabled — always unavailable, so the orchestrator
    (parser.py) falls back to the rules-only result at zero cost/latency."""
    raise LLMParserUnavailable("LLM resume parsing is disabled (OpenAI API not in use)")
