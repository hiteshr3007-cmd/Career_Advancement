  ...
  return (
    <div className="space-y-6">
      <div className="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm">
        ...
        {report.narrative && (
          <p className="mt-4 text-sm text-slate-600">{report.narrative}</p>
        )}
      </div>
      ...
      <section className="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm">
        ...
        {report.skill_gaps.length === 0 ? (
          <p className="mt-2 text-sm text-slate-500">No prioritized skill gaps found.</p>
        ) : (
      ...
      <section className="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm">
        ...
        {report.resume_issues.length === 0 ? (
          <p className="mt-2 text-sm text-slate-500">No resume issues detected.</p>
        ) : (
          <ul className="mt-3 space-y-3">
            {report.resume_issues.map((issue: ResumeIssue, index: number) => (
              <li key={`${issue.type}-${index}`} className="rounded-xl border border-slate-100 p-3">
                <div className="flex flex-wrap items-center gap-2">
                  <Badge variant={BAND_VARIANT[issue.severity] ?? "outline"}>
                    {issue.severity}
                  </Badge>
                  <span className="text-sm font-medium text-slate-900">{issue.detail}</span>
                </div>
